...
use Modules\Manage\Repositories\DashboardRepository as DashboardRepository;
...
class DashboardController extends UploadeFileController
{
    ...
    public function index()
    {
        ...
        $data['ac'] = $this->DashboardRepository->index('activity','id_districts',Auth::user()->id);

        $data['re'] = $this->DashboardRepository->index('reportmeeting','id_user',Auth::user()->id);

        $data['mee'] = $this->DashboardRepository->index('meeting','id_user',Auth::user()->id);

        $data['sumwithdraw'] = \DB::table('reportactivity')
                        ->select(\DB::raw("SUM(re_ac_withdraw) as sumwithdraw"))
                        ->where('id_user_reac',Auth::user()->id)
                        ->get()[0];
